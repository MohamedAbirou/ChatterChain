...
const Form = () => {
    ...
    return (
        <div className="py-4 px-4 bg-white border-t flex items-center gap-2 lg:gap-4 w-full">
            <CldUploadButton options={{ maxFiles: 1 }} onUpload={handleUpload} uploadPreset="nhshcqp6">
                <HiPhoto size={30} className="text-emerald-500" />
            </CldUploadButton>
            <form onSubmit={handleSubmit(onSubmit)} className="flex items-center gap-2 lg:gap-4 w-full">
                <MessageInput id="message" register={register} errors={errors} required placeholder="Write a message" />
                <EmojiPicker
                    onChange={(emoji: string) => setValue('message', `${getValues('message')} ${emoji}`)}                    
                />
